...
assert.ok($('.pv-player'));$('.pv-player').click();assert.ok($('.pv-strip').classList.contains('pv-up'));assert.ok($('.pv-groups'));
...
$('[data-mode="multi"]').click();
const saved=Object.entries(dom.window.localStorage),refreshURL=dom.window.location.href;
assert.equal(new URL(refreshURL).searchParams.get('view'),'multi');
const next=new JSDOM(await readFile(new URL('../public/index.html',import.meta.url),'utf8'),{url:refreshURL,runScripts:'outside-only',pretendToBeVisual:true});
Object.assign(next.window,{createWatch,...scoring,...video,ResizeObserver:dom.window.ResizeObserver,matchMedia:dom.window.matchMedia,backend:dom.window.backend});
for(const [key,value] of saved)next.window.localStorage.setItem(key,value);
dom.window.close();globalThis.document=next.window.document;
next.window.eval((await readFile(new URL('../public/app.js',import.meta.url),'utf8')).replace(/^import .*;\n/gm,''));
await new Promise(r=>setTimeout(r,600));
assert.ok(next.window.document.querySelector('.watch-surface.w-is-multi'));
assert.equal(new URL(next.window.location.href).searchParams.get('view'),'multi');
console.log('App boot, multiview, return to roster and full-page Multi refresh passed.');next.window.close();
